...
class ActionLog{
    ...
    constructor({actions:actions,meta:meta}) {
        console.log('^Class ActionLog^constructor^actions:'+AKT.simpleStringify(actions)+'; meta:'+AKT.simpleStringify(meta));
    ...
    display () {
        var newScriptWindow = window.open("", "_blank");

        // Add content to the new window
        if (newScriptWindow) {
            newScriptWindow.document.write('<pre>'+JSON.stringify(this._actions,null,4)+'</pre>');
            newScriptWindow.document.close(); // Close the document stream to render the content
        } else {
    ...
    stepThroughRecording (id) {
        ...
        if (!AKT.state.startedStepping) {
            ...
            var firstEventMessage = this._actions[0].speech;
        ...
        if (iStep < this._actions.length) {
            var nextEventMessage = this._actions[iStep].speech;
    ...
    play (id) {
        ...
        var timestep = Number($('#input_actionscript_replay_timestep').val());
        if (timestep>=7) {
            AKT.state.actionscript_replay_speech = true;
        } else {
            AKT.state.actionscript_replay_speech = false;
        }
        // recorded themselves.
        //if (!id) {
        //    var fromLocalStorage = JSON.parse(localStorage.getItem('current_action_log'));
        ...
        AKT.state.stepCounter = 0;
        var nextAction = this._actions[0];
        console.log('play 3',nextAction);
        AKT.text_to_speech('Welcome to this web AKT demo, called add source and statement.  '+nextAction.speech);
        console.log('play 4');
        AKT.timer = setInterval(this.oneStep,timestep*1000,this._actions);
    ...
    oneStep () {
        ...
        var istep = AKT.state.stepCounter;
        console.log('oneStep 2',istep);
        if (istep === 3) {
            //$( "#test_alert" ).css({display:'block'});
        }
        var actionLog = AKT.state.current_action_log;
        //$('#message').html(actions[istep].before);
        console.log('oneStep 3',actionLog);
    ...
    triggerEvent (istep) {
        console.log('   triggerEvent 1: ',istep);
        var action = this._actions[istep];
        if (istep < this._actions.length-1) {
            //var nextAction = this._actions[istep+1];
            //AKT.text_to_speech(nextAction.speech);
        }
        ...
        console.log('   triggerEvent 2: ',istep,': ',eventType,': ',elementId,': ',selector,': ',value);
        if (eventType === 'click'){
            ...
            if (action.upper_selector && !action.local_selector) {
                ...
                console.log('   triggerEvent 3: ',upperSelector,': ',localSelector);
                $(upperSelector).find(localSelector).focus();  // This is essential for menus! ... since menus are
                    // pure CSS, and respond to user click by becoming active.  See the CSS to find the relevant rule.
