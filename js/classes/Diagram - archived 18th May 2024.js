class Diagram {
    ...
    constructor(id,language,spec) {
        ...
        if (language === 'systo') {
            if (spec) {
                this._subgraph.meta = spec.meta;
                this._subgraph.nodes = spec.nodes;
                this._subgraph.arcs = spec.arcs;
            } else {
    ...
    convertJointToSysto = function () {
        ...
        console.log(JSON.stringify(this._subgraph));
...
//  and adds them to the JointJS graph.
    ...
    convertSystoToJoint = function (subgraph) {
        ...
        for (var arcId in subgraph.arcs) {
            ...
            if (arc.aktType !== 'condition'){
                //console.log('non-condition arc:',arc);
                if (arc) {
...
*/
    ...
    createJlink = function (arc) {
        ...
        //try {
            var aktType = arc.type;   // TODO: fix this temporary hack.
            //aktType = 'causes1way';
            var aktId = arc.id;
            var sourceId = arc.start_node.id;

            // Hde all vertex nodes (used as target for condition arc).
...
*/
    ...
    makeSubgraph = function (searchExpression,show) {
        ...
        var filters = {type:{causal:true},topic:searchExpression};
