AKT.widgets.diagram_from_topic = {};


AKT.widgets.diagram_from_topic.setup = function (widget) {
...
// =========================== LISTBOX HANDLERS  ==============================
    ...
    $(widget.element).find('.button_display').on('click', function() {
        ...
        diagram._subgraph = diagram.makeSubgraph(topicId);
        ...
        if (Object.keys(diagram._subgraph.arcs).length > 0) {
            ...
            console.log('diagram: ',diagram);
            var i=0;
            for (var arcId in diagram._subgraph.arcs) {
                i += 1;
                var arc = diagram._subgraph.arcs[arcId];
                //console.log(i,arcId,arc.start_node_id, arc.end_node_id);
            }
            var i=0;
            for (var nodeId in diagram._subgraph.nodes) {
                i += 1;
                var node = diagram._subgraph.nodes[nodeId];
...
// ===========================  JOINT HANDLERS  ===============================
    ...
    paper.on('link:mouseenter', function(linkView) {
        ...
        // TODO 25 Aug 2023 $(widget.element).find('.popup').html(AKT.KBs[kbId]._statements[linkView.model.id].english);
    });
...
AKT.widgets.diagram_from_topic.display = function (widget) {
...
AKT.widgets.diagram_from_topic.html = `
