...
21 Aug 2023
This version (node_template.js) presents the template appropriate to the user's choice of
node.  See node_template_x1.js for the previous pattern.
*/
...
*/
    ...
    $(widget.element).find('.button_accept').on('click', function() {
        AKT.recordEvent({
            ...
            event:'click',
            message:'Clicked the Accept button in the node_template widget'});
        ...
        for (var i=0;i<array.length;i++) {
            var a = array[i];
            if (a!=='attribute' && a!=='part' && a!=='process' && a!=='action') {
                if (id==='') {
                    id += a;
                } else {
                    id += '_'+a;
...
*/
    ...
    $(widget.element).find('.button_ok').on('click', function() {
        ...
        AKT.trigger('node_created event',{id:'fred'});
...
*/
    ...
    $(widget.element).find('.attribute_options input').on('change',function(event,attributeType) {
        ...
        AKT.recordEvent({
            ...
            event:   'change',
            value:   event.target.value,
            message: 'Clicked an attribute_options radiobutton in the node_template widget'});
        $(this).attr('checked',true);
